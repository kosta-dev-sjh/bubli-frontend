...
| 라우트 연결 | 약 95% | 공개 사이트, 로그인, `/app`, 프로젝트룸, 자료보드, 작업 화면, 채팅, 설정, 데스크탑 소통/버블 route가 존재한다. 세부 화면 데이터 연결은 API 계약 뒤 작업이다. |
...
| 디자인보드 v20 정합화 | 약 49% | 공통 버블 질감, 일부 한국어 줄바꿈, Tauri 소통 패널, 대시보드/하루정리, 위젯/설정, 자료/개인 에이전트, 타이머/기기 복구/초대 권한, 관리 폴더/자료 업로드, 에이전트 후보/재시도, 인증 세션/공개 FAQ, 공개/다운로드 하이브리드 앱, 소통/실시간 연결, 설정/기기 권한/위젯, 최신 결정 충돌 후보 사용자 문구, 설정/대시보드/자료보드/프로젝트룸/소통/데스크탑 버블/로그인/공개 기능 화면 정보 밀도, 설정 기본 카드 문구, 설정 라우트 정보 밀도는 보정됐지만 전체 화면 질감은 아직 2차 polish가 필요하다. |
...
2. 자료보드, 대시보드, 프로젝트룸, 소통, 버블 순서로 실제 라우트를 재조립한다.
