...
디자인보드 v20 정합화 전까지 새 색상이 임의로 늘어나지 않도록 `npm run check:design-tokens`를 CI에 포함한다. 이 검사는 `src`와 `.storybook` 안에서 직접 쓰인 hex color가 Bubli 토큰 allowlist 안에 있는지 확인한다.
...
| 디자인보드 v20 정합화 | 약 51% | 공통 버블 질감과 일부 문구는 보정했고, 새 hex color가 임의로 늘어나지 않도록 디자인 토큰 검사를 CI에 넣었다. 디자인 전면 개편은 P2에서 클로드 작업 결과와 함께 크게 반영한다. |
