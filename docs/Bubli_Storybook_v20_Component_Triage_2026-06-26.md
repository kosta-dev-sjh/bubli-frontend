...
| Storybook 대표 점검 | 약 54% | 대표 story 24개 기준 중 일부는 캡처와 문서 점검이 끝났고, 에이전트 결과 검증, 소통 경계 문구, 대시보드/하루정리 문구, 위젯/기기 저장소 문구, 자료/개인 에이전트 문구와 실제 라우트 예시 문구까지 추가 보정했다. 남은 story는 계속 순차 점검해야 한다. |
| 디자인보드 v20 정합화 | 약 31% | 공통 버블 질감, 일부 한국어 줄바꿈, Tauri 소통 패널, 대시보드/하루정리, 위젯/설정, 자료/개인 에이전트 사용자 문구와 일부 라우트 문구는 보정됐지만 전체 화면 밀도와 질감은 아직 2차 polish가 필요하다. |
| API 계약 반영 | 약 20% | API client 위치와 일부 타입은 있으나, 확정 DTO와 payload가 오면 교체해야 한다. |
| Tauri 경계 검증 | 약 35% | Tauri IPC wrapper와 관리 폴더/위젯/소통 경계 story는 있고, 소통 창/버블과 기기 안 저장소의 책임 경계 문구를 1차 보정했다. 실제 앱 연결 검증은 더 필요하다. |
