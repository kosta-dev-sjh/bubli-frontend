...
현재 상태는 `Bubli_프론트엔드_개발계획_AtoZ_2026-06-26.md` 기준으로 **1차 컴포넌트 추출 약 80%, P0 진행 중**이다. 아래 진행도는 전체 제품 완료율이 아니라, Storybook과 라우트에 드러난 작업량을 기준으로 잡은 관리용 추정치다.

## 1. 현재 Storybook 규모

| 구분 | 개수 | 판단 |
|---|---:|---|
| 전체 Story | 145개 | 1차 점검판 기준으로 많다. 최종 화면 수가 아니라 기능 후보와 경계 검증용 story까지 포함한다. |
| `src/features` Story | 137개 | 기획 기능, API 경계, Tauri 경계, 예외 상태를 보여주는 story다. |
| `src/components` Story | 8개 | 공통 UI, 도메인 카드, 버블, 레이아웃 기준 story다. |
| 실제 page route | 14개 | 공개 사이트, 로그인, 회원 앱, 데스크탑 전용 화면, 일정 화면까지 있다. |
| feature component | 136개 | v15 핵심 기능은 1차 화면 조각으로 대부분 뽑혔다. |
| component inventory | 149행 | 현재 story 145개와 AtoZ 누락 4종을 함께 분류했다. |
| 미완료 | 4개 | 에이전트 제안함, 대시보드 5카드, WBS 4보기, 위젯 8종이다. |
...
| 작업 | 진행도 | 근거 |
|---|---:|---|
| 1차 컴포넌트 추출 | 약 84% | Storybook story 145개와 feature component 137개가 있고, AtoZ 기준 누락 5종 중 캘린더 라우트/4상태 골격을 추가했다. 남은 미완료는 4종이다. |
| 라우트 연결 | 약 92% | 공개 사이트, 로그인, `/app`, 프로젝트룸, 자료보드, 작업 화면, 채팅, 일정, 설정, 데스크탑 소통/버블 route는 있다. 전역 제안함 노출은 아직 필요하다. |
...
1. `docs/component-inventory.csv`의 미완료 4종을 먼저 구현한다.
2. 에이전트 제안함, 대시보드 5카드, WBS 4보기, 위젯 8종 순서로 골격과 story를 채운다.
3. 점검용 패널은 삭제하지 않고 Storybook에 남긴다.
4. P0 완료 뒤 P1 전 화면 노출로 넘어간다.
