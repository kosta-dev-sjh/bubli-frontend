...
이 문서는 1차로 뽑힌 컴포넌트를 모두 최종 화면으로 보지 않기 위한 분류표다. 지금 단계의 목적은 컴포넌트를 더 많이 만드는 것이 아니라, 최신 기획과 충돌하지 않는 화면 조각을 Storybook에서 확인하고 디자인보드 v20 전면개편 때 합치거나 버릴 수 있는 상태로 두는 것이다.

현재 상태는 `Bubli_1차_컴포넌트_추출_완료점검_2026-06-26.md` 기준으로 **1차 컴포넌트 추출 완료 기준을 통과한 상태**다. 아래 진행도는 전체 제품 완료율이 아니라, Storybook과 라우트에 드러난 작업량을 기준으로 잡은 관리용 추정치다.
...
| 실제 page route | 13개 | 공개 사이트, 로그인, 회원 앱, 데스크탑 전용 화면까지 있다. |
| feature component | 136개 | v15 핵심 기능은 1차 화면 조각으로 대부분 뽑혔다. |
...
| 작업 | 진행도 | 근거 |
|---|---:|---|
| 1차 컴포넌트 추출 | 약 95% | page route 13개, feature component 136개, Storybook story 144개가 있고 v15 핵심 화면은 1차 추출 완료 기준을 통과했다. 남은 5%는 API 계약 확정 뒤 필요한 작은 상태 컴포넌트와 중복 병합이다. |
| 라우트 연결 | 약 96% | 공개 사이트, 로그인, `/app`, 프로젝트룸, 자료보드, 작업 화면, 채팅, 설정, 데스크탑 소통/버블 route가 존재한다. 자료보드는 점검용 패널을 내리고 실제 사용 흐름 중심으로 1차 재조립했다. 세부 화면 데이터 연결은 API 계약 뒤 작업이다. |
...
| 디자인보드 v20 정합화 | 약 52% | 공통 버블 질감, 일부 한국어 줄바꿈, Tauri 소통 패널, 대시보드/하루정리, 위젯/설정, 자료/개인 에이전트, 타이머/기기 복구/초대 권한, 관리 폴더/자료 업로드, 에이전트 후보/재시도, 인증 세션/공개 FAQ, 공개/다운로드 하이브리드 앱, 소통/실시간 연결, 설정/기기 권한/위젯, 최신 결정 충돌 후보 사용자 문구, 설정/대시보드/자료보드/프로젝트룸/소통/데스크탑 버블/로그인/공개 기능 화면 정보 밀도, 설정 기본 카드 문구, 설정 라우트 정보 밀도, 자료보드 실제 라우트 1차 재조립까지 보정했다. 전체 화면 질감은 아직 2차 polish가 필요하다. |
...
1. 새 컴포넌트를 더 늘리기보다 디자인보드 v20 기준 공통 토큰과 기본 컴포넌트를 먼저 정리한다.
2. 자료보드 1차 재조립 결과를 기준으로 대시보드, 프로젝트룸, 소통, 버블 순서로 실제 라우트를 재조립한다.
3. 최신 기획과 충돌하는 문구나 흐름은 즉시 고친다.
4. 중복이 의심되는 story는 이 문서의 병합 후보나 삭제 검토 후보에 먼저 넣는다.
5. API 계약이 확정되면 화면을 크게 고치기 전에 feature API/client 타입을 먼저 맞춘다.
