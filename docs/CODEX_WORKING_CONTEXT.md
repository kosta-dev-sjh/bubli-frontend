...
- 이번 검증:
  - `npm run typecheck` 통과
  - `git diff --check` 통과
- 개인 대시보드 참고 요소를 추가로 고정했다.
- 참고한 방향은 그대로 복붙이 아니라 요소 단위 차용이다.
- 대시보드 카드 후보:
  - 주간/오늘 활동 링: 할 일, 일정, 확인 필요, 버블/집중 시간처럼 개인 누적 지표를 작은 원형 진행률로 보여준다.
  - 자주 반복된 작업 의도: 이번 주 많이 다룬 프로젝트룸, 자주 확인한 자료, 반복된 에이전트 후보를 행 목록으로 보여준다.
  - 습관/연속 기록: 작업 시작일, 일정 처리, 확인 완료처럼 개인 흐름을 달력형/연속형 지표로 보여준다.
- 금지:
  - 날씨, 오디오, 영양 정보처럼 Bubli 업무와 무관한 원본 예시 항목은 쓰지 않는다.
  - 모바일 앱 시안처럼 하단 마이크 버튼을 그대로 가져오지 않는다.
  - 대시보드가 랜딩/요약 소개 페이지처럼 보이면 안 된다.
