...
- 선택한 WBS는 이름, 상태, 줄 색을 바로 수정할 수 있어야 한다.
- WBS 자체 기한은 별도 필드로 만들지 않는다. 기간 보기는 연결된 TODO의 `dueAt`을 기준으로 계산한다.
...
- 작업 카드는 화면에서만 숨기는 가짜 제거 기능을 두지 않는다. 실제 상태 변경은 칸반 드래그와 상태 버튼으로 처리한다.
