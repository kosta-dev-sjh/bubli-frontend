...
- WBS 타임라인은 자동 무한 확장으로 사용자를 오른쪽 끝으로 밀지 않는다. 필요한 범위 안에서 안정적으로 스크롤을 유지한다.
- WBS 줄을 클릭하면 수정창을 열지 않고, 오른쪽 타임라인의 해당 바 위치로 이동한다. 수정은 연필 버튼으로만 연다.
- 상위 WBS는 하위 작업을 접고 펼칠 수 있어야 한다. 접으면 왼쪽 목록과 오른쪽 바가 함께 숨겨져야 한다.
