...
3. `src/lib/api`에 공통 응답 포맷과 인증 헤더 기준을 만든다.
4. 프로젝트룸, 자료보드, WBS/작업판, 대시보드 화면의 빈 상태를 만든다.
5. 채팅과 보이스챗은 웹 `/app/chat`에서 먼저 붙이고, Tauri 전용 창은 같은 연결을 재사용한다.
6. Tauri IPC는 화면 연결 뒤 `src/lib/tauri`와 `src-tauri/src`에서 붙인다.
7. 버블 위젯은 회원 웹 화면 복제가 아니라 개인 작업 인터페이스로 따로 구성한다.
