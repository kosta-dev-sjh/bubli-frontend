...
| 공개 기능 화면 정보 밀도 | `/features`에 섞여 있던 글자 전략 패널은 Storybook 점검용으로 남기고 실제 공개 기능 라우트에서는 제외했다. 공개 기능 화면은 프로젝트룸, 자료보드, 에이전트 후보, WBS/작업판, 소통, 버블의 제품 흐름 중심으로 둔다. |
| 설정 기본 카드 문구 | 사용자 설정 카드에 남아 있던 API 후보 경로는 Storybook/개발 점검용 표현으로 보고 실제 설정 라우트에서는 제거했다. 설정 화면은 프로필, 화면 표시, 개인 기능처럼 사용자가 이해하는 저장 기준을 먼저 보여준다. |
| 설정 라우트 2차 밀도 | `/app/settings`에 남아 있던 알림 라우팅, 접근 경계, 접근성, 기기 복구 지도, 관리 폴더 정책/반영, 일정 충돌, 타이머 복구, 전송 대기열 패널은 Storybook 점검용으로만 두었다. 실제 설정 라우트는 사용자 기본 설정, 표시, 알림, 기기 기능 동의, 개인 관리 폴더, Google Calendar, 백업, 삭제 요청 중심으로 정리했다. |
| 1차 컴포넌트 추출 기준 | page route 13개, feature component 136개, Storybook story 144개 기준으로 v15 핵심 화면 조각은 1차 추출 완료 기준을 통과했다. 이후 작업은 새 컴포넌트 양산이 아니라 디자인보드 v20 기준 재조립과 API 계약 반영으로 전환한다. |
| 자료보드 실제 라우트 재조립 | `/app/resources`에서 자료 권한/업로드 결정/개인 공유 경계 같은 점검용 패널을 제외하고, 자료 목록, 검색, 업로드 분석, 문서 분류, 후보 승인, 확인 필요 항목, 관련 문서, 버전, 댓글, 공유 흐름으로 다시 묶었다. |
