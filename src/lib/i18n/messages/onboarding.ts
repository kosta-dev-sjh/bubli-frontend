// 첫 사용 경험 네임스페이스, 직군 온보딩 오버레이 + 워크스페이스 튜토리얼(코치 마크).
const ko = {
  // 직군 온보딩 오버레이
  "onboarding.aria": "첫 사용 설정",
  "onboarding.stepLabel": "{current} / {total} 단계",
  "onboarding.back": "이전",
  "onboarding.next": "다음",
  "onboarding.skip": "건너뛰기",
  "onboarding.welcome.kicker": "Bubli 시작하기",
  "onboarding.welcome.title": "만나서 반가워요, {name}님!",
  "onboarding.welcome.body": "버블리는 프로젝트룸, 일정, AI 후보함을 하나의 워크스페이스로 모아요. 1분이면 나에게 맞는 홈을 준비할 수 있어요.",
  "onboarding.welcome.nameLabel": "표시 이름",
  "onboarding.welcome.nameHint": "홈과 프로젝트룸에서 이 이름으로 보여요. 설정에서 언제든 바꿀 수 있어요.",
  "onboarding.role.title": "어떤 일을 하세요?",
  "onboarding.role.body": "직군에 맞춰 홈 카드 구성과 기본 시작 화면을 추천해 드려요.",
  "onboarding.role.gridAria": "직군 선택",
  "onboarding.role.developer": "개발자",
  "onboarding.role.designer": "디자이너",
  "onboarding.role.pm": "PM, 기획",
  "onboarding.role.marketer": "마케터",
  "onboarding.role.writer": "작가, 콘텐츠",
  "onboarding.role.etc": "기타",
  "onboarding.preset.title": "{role} 맞춤 세팅이 준비됐어요",
  "onboarding.preset.body": "아래 구성으로 홈을 시작해요. 홈의 카드 편집에서 언제든 바꿀 수 있어요.",
  "onboarding.preset.cardsLabel": "홈 카드 구성",
  "onboarding.preset.homeLabel": "기본 시작 화면",
  "onboarding.preset.homePersonal": "개인 홈",
  "onboarding.preset.homeProjectRoom": "프로젝트룸",
  "onboarding.preset.apply": "이 구성으로 시작",
  "onboarding.preset.applying": "적용 중…",

  // 위젯 초기 진입 온보딩 오버레이
  "onboarding.desktopOverlay.aria": "위젯 첫 실행 가이드",
  "onboarding.desktopOverlay.progress": "{current} / {total}단계",
  "onboarding.desktopOverlay.next": "둘러보기",
  "onboarding.desktopOverlay.skip": "건너뛰기",
  "onboarding.desktopOverlay.done": "시작하기",
  "onboarding.desktopOverlay.welcome.title": "버블리에 오신 걸 환영해요",
  "onboarding.desktopOverlay.welcome.body": "일을 방해하지 않는 데스크탑 도우미예요. 30초면 핵심만 짚어드릴게요.",
  "onboarding.desktopOverlay.bubble.title": "방울, 언제나 떠 있는 시작점",
  "onboarding.desktopOverlay.bubble.body": "화면 위를 둥둥 떠다니는 동그란 방울이에요. 클릭하면 메뉴가 열려요.",
  "onboarding.desktopOverlay.bar.title": "바, 접어둔 위젯이 모이는 곳",
  "onboarding.desktopOverlay.bar.body": "최소화한 위젯은 여기 담겨요. 클릭하면 펼쳐지고, 올려두면 미리보기만 떠요. (알림은 예외로 바로 떠요.)",
  "onboarding.desktopOverlay.room.title": "개인과 프로젝트룸, 구분돼요",
  "onboarding.desktopOverlay.room.body": "타이머, 메모는 개인 전용이에요. 소통 같은 위젯은 프로젝트룸에 속하고요. 룸은 꼭 고르지 않아도 괜찮아요.",
  "onboarding.desktopOverlay.modes.title": "기본과 반투명과 고스트",
  "onboarding.desktopOverlay.modes.body": "위젯 투명도를 3단계로 바꿀 수 있어요. 고스트는 숫자 같은 핵심만 유리처럼 남겨서 일에 방해되지 않아요.",
  "onboarding.desktopOverlay.environment.title": "내 작업 환경을 골라주세요",
  "onboarding.desktopOverlay.environment.body": "위젯을 어떻게 펼쳐둘지 고르면 그대로 구성해 드려요. 버블 메뉴의 자동 정렬에서 언제든 바꿀 수 있어요.",
  "onboarding.desktopOverlay.environment.aria": "시작 환경 선택",
  "onboarding.desktopOverlay.environment.bar.title": "미니멀 바",
  "onboarding.desktopOverlay.environment.bar.body": "얇은 바 하나만 두고 필요할 때 펼쳐 써요.",
  "onboarding.desktopOverlay.environment.board.title": "위젯 보드",
  "onboarding.desktopOverlay.environment.board.body": "할 일과 일정과 타이머를 화면 오른쪽에 정리해 둬요.",
  "onboarding.desktopOverlay.environment.cascade.title": "카드 덱",
  "onboarding.desktopOverlay.environment.cascade.body": "핵심 위젯 세 개를 계단처럼 모아둬요.",
  "onboarding.desktopOverlay.ready.title": "준비 끝!",
  "onboarding.desktopOverlay.ready.body": "필요하면 설정 > 표시에서 다시 볼 수 있어요.",

  // 워크스페이스 튜토리얼(코치 마크)
  "tour.aria": "워크스페이스 둘러보기",
  "tour.progress": "{current} / {total}",
  "tour.next": "다음",
  "tour.prev": "이전",
  "tour.skip": "건너뛰기",
  "tour.done": "시작하기",
  "tour.welcome.title": "버블리에 오신 걸 환영해요",
  "tour.welcome.body": "버블리는 흩어진 프로젝트 자료와 할 일과 일정을 한곳에 모아주는 프리랜서 업무 허브예요. 주요 메뉴를 하나씩 소개해 드릴게요.",
  "tour.sidebar.title": "사이드바",
  "tour.sidebar.body": "왼쪽 사이드바에서 앱의 모든 화면을 오갈 수 있어요. 주요 탭을 하나씩 짚어드릴게요.",
  "tour.calendar.title": "일정",
  "tour.calendar.body": "마감과 약속을 한눈에 보고, 여기서 바로 새 일정을 잡아요.",
  "tour.projectRooms.title": "프로젝트룸",
  "tour.projectRooms.body": "팀과 함께 쓰는 작업 공간이에요. 룸마다 할 일·자료·소통이 모여요.",
  "tour.resources.title": "자료보드",
  "tour.resources.body": "계약서·이미지 같은 파일을 올려두고 팀과 공유해요.",
  "tour.chat.title": "소통",
  "tour.chat.body": "프로젝트룸 팀원과 메시지와 음성으로 대화해요.",
  "tour.roomSwitcher.title": "프로젝트룸 선택",
  "tour.roomSwitcher.body": "상단에서 지금 작업 중인 프로젝트룸을 바꾸거나 새로 만들 수 있어요.",
  "tour.cardEdit.title": "홈 카드 편집",
  "tour.cardEdit.body": "홈 카드는 끌어서 순서를 바꾸고, 필요한 카드만 남길 수 있어요.",
  "tour.agent.title": "AI 후보함",
  "tour.agent.body": "AI가 제안한 할 일과 일정 후보가 여기에 모여요. 확인하고 승인하면 반영돼요.",

  // 설정 > 표시, 회원사이트 튜토리얼 다시 보기
  "onboarding.settings.replayTitle": "회원사이트 튜토리얼 다시 보기",
  "onboarding.settings.replayDesc": "워크스페이스 안내(사이드바, 프로젝트룸, 홈 카드, AI 후보함)를 처음부터 다시 봅니다.",
  "onboarding.settings.replayCta": "다시 보기",

  // 위젯 전용 전체화면 튜토리얼
  "widgetTutorial.aria": "위젯 튜토리얼",
  "widgetTutorial.badge": "위젯 튜토리얼",
  "widgetTutorial.skip": "건너뛰기",
  "widgetTutorial.prev": "이전",
  "widgetTutorial.next": "다음",
  "widgetTutorial.done": "시작하기",
  "widgetTutorial.welcome.title": "화면 위의 작은 작업실, 버블",
  "widgetTutorial.welcome.body": "버블리 위젯은 브라우저를 열지 않아도 오늘 할 일·타이머·메모를 화면 위에 띄워 둡니다. 30초면 핵심만 짚어드릴게요.",
  "widgetTutorial.bubble.title": "방울 — 언제나 떠 있는 시작점",
  "widgetTutorial.bubble.body": "동그란 방울은 화면 어디든 두고, 클릭하면 메뉴가 열립니다. 드래그로 원하는 자리에 옮겨 두세요.",
  "widgetTutorial.bar.title": "바 — 접어둔 위젯이 모이는 곳",
  "widgetTutorial.bar.body": "쓰던 위젯을 접으면 얇은 바에 칩으로 모입니다. 칩 위에 올리면 미리보기만 살짝 떠요.",
  "widgetTutorial.room.title": "개인과 프로젝트룸이 구분돼요",
  "widgetTutorial.room.body": "상단에서 개인/프로젝트룸을 바꾸면 위젯 내용도 그 컨텍스트로 바뀝니다. 타이머·메모 같은 건 개인 전용이에요.",
  "widgetTutorial.modes.title": "기본·반투명·고스트로 투과 조절",
  "widgetTutorial.modes.body": "작업에 방해되지 않게 위젯 투명도를 3단계로 바꿀 수 있어요. 고스트는 숫자·핵심만 유리처럼 남깁니다.",
  "widgetTutorial.ready.title": "준비 끝!",
  "widgetTutorial.ready.body": "설정 > 표시에서 위젯 튜토리얼을 언제든 다시 볼 수 있어요.",
  "widgetTutorial.scene.default": "기본",
  "widgetTutorial.scene.translucent": "반투명",
  "widgetTutorial.scene.ghost": "고스트",
  "widgetTutorial.scene.personal": "개인",
  "widgetTutorial.scene.room": "프로젝트룸",

  // 설정 > 표시, 위젯 튜토리얼 다시 보기
  "onboarding.settings.widgetReplayTitle": "위젯 튜토리얼 다시 보기",
  "onboarding.settings.widgetReplayDesc": "데스크탑 버블(방울·바·개인/룸·투과 모드) 사용법을 전체화면으로 다시 봅니다.",
  "onboarding.settings.widgetReplayCta": "다시 보기",
} as const;

type Key = keyof typeof ko;

const en: Record<Key, string> = {
  "onboarding.aria": "First-run setup",
  "onboarding.stepLabel": "Step {current} of {total}",
  "onboarding.back": "Back",
  "onboarding.next": "Next",
  "onboarding.skip": "Skip",
  "onboarding.welcome.kicker": "Getting started with Bubli",
  "onboarding.welcome.title": "Nice to meet you, {name}!",
  "onboarding.welcome.body": "Bubli brings project rooms, schedules, and the AI inbox into one workspace. One minute is all it takes to set up a home that fits you.",
  "onboarding.welcome.nameLabel": "Display name",
  "onboarding.welcome.nameHint": "This is how you appear on home and in project rooms. You can change it anytime in Settings.",
  "onboarding.role.title": "What kind of work do you do?",
  "onboarding.role.body": "We recommend home cards and a default start screen that match your role.",
  "onboarding.role.gridAria": "Choose your role",
  "onboarding.role.developer": "Developer",
  "onboarding.role.designer": "Designer",
  "onboarding.role.pm": "PM, Planner",
  "onboarding.role.marketer": "Marketer",
  "onboarding.role.writer": "Writer, Content",
  "onboarding.role.etc": "Other",
  "onboarding.preset.title": "Your {role} setup is ready",
  "onboarding.preset.body": "Your home starts with the layout below. You can change it anytime with Edit cards on home.",
  "onboarding.preset.cardsLabel": "Home cards",
  "onboarding.preset.homeLabel": "Default start screen",
  "onboarding.preset.homePersonal": "Personal home",
  "onboarding.preset.homeProjectRoom": "Project room",
  "onboarding.preset.apply": "Start with this setup",
  "onboarding.preset.applying": "Applying…",

  "onboarding.desktopOverlay.aria": "Widget startup guide",
  "onboarding.desktopOverlay.progress": "Step {current} of {total}",
  "onboarding.desktopOverlay.next": "Look around",
  "onboarding.desktopOverlay.skip": "Skip",
  "onboarding.desktopOverlay.done": "Get started",
  "onboarding.desktopOverlay.welcome.title": "Welcome to Bubli",
  "onboarding.desktopOverlay.welcome.body": "It's a desktop assistant that doesn't get in your way. We'll cover the essentials in 30 seconds.",
  "onboarding.desktopOverlay.bubble.title": "Bubble, your always-visible anchor",
  "onboarding.desktopOverlay.bubble.body": "A soft round orb that always floats on top. Click it to open the menu.",
  "onboarding.desktopOverlay.bar.title": "Bar, where minimized bubbles gather",
  "onboarding.desktopOverlay.bar.body": "Minimized bubbles are collected here. Click to expand, hover to only preview. (Alerts pop open immediately.)",
  "onboarding.desktopOverlay.room.title": "Personal and project room modes are separate",
  "onboarding.desktopOverlay.room.body": "Timer and memo are personal-only. Communication like chat belongs to your project room. You can skip selecting a room.",
  "onboarding.desktopOverlay.modes.title": "Default, translucent, and ghost",
  "onboarding.desktopOverlay.modes.body": "Dial widget opacity across three levels. Ghost keeps only the essentials like numbers, so it never gets in your way.",
  "onboarding.desktopOverlay.environment.title": "Choose your workspace setup",
  "onboarding.desktopOverlay.environment.body": "Pick how the widgets should start and we'll arrange them for you. Change it anytime from auto-arrange in the bubble menu.",
  "onboarding.desktopOverlay.environment.aria": "Startup layout choice",
  "onboarding.desktopOverlay.environment.bar.title": "Minimal bar",
  "onboarding.desktopOverlay.environment.bar.body": "Keep just the slim bar and expand widgets when needed.",
  "onboarding.desktopOverlay.environment.board.title": "Widget board",
  "onboarding.desktopOverlay.environment.board.body": "Tasks, schedule, and timer arranged on the right side of your screen.",
  "onboarding.desktopOverlay.environment.cascade.title": "Card deck",
  "onboarding.desktopOverlay.environment.cascade.body": "Three core widgets gathered like a tidy stack of cards.",
  "onboarding.desktopOverlay.ready.title": "You are ready!",
  "onboarding.desktopOverlay.ready.body": "If needed, you can open this again from Settings > Display.",

  "tour.aria": "Workspace tour",
  "tour.progress": "{current} / {total}",
  "tour.next": "Next",
  "tour.prev": "Back",
  "tour.skip": "Skip",
  "tour.done": "Get started",
  "tour.welcome.title": "Welcome to Bubli",
  "tour.welcome.body": "Bubli is a freelancer work hub that gathers scattered project files, tasks, and schedules in one place. Let us walk you through the main menus.",
  "tour.sidebar.title": "Sidebar",
  "tour.sidebar.body": "Move between every screen from the left sidebar. Let's walk through the main tabs one by one.",
  "tour.calendar.title": "Calendar",
  "tour.calendar.body": "See deadlines and appointments at a glance, and add new events right here.",
  "tour.projectRooms.title": "Project rooms",
  "tour.projectRooms.body": "Shared workspaces for your team. Each room gathers its tasks, files, and chat.",
  "tour.resources.title": "Resource board",
  "tour.resources.body": "Upload files like contracts and images, and share them with your team.",
  "tour.chat.title": "Chat",
  "tour.chat.body": "Talk with your project room teammates by message and voice.",
  "tour.roomSwitcher.title": "Project room switcher",
  "tour.roomSwitcher.body": "Switch the project room you are working in, or create a new one, from the top bar.",
  "tour.cardEdit.title": "Edit home cards",
  "tour.cardEdit.body": "Drag home cards to reorder them and keep only the cards you need.",
  "tour.agent.title": "AI inbox",
  "tour.agent.body": "Todo and schedule candidates suggested by AI gather here. Review and approve them to apply.",

  "onboarding.settings.replayTitle": "Replay member-site tutorial",
  "onboarding.settings.replayDesc": "Watch the workspace walkthrough (sidebar, project rooms, home cards, AI inbox) again from the start.",
  "onboarding.settings.replayCta": "Replay",

  "widgetTutorial.aria": "Widget tutorial",
  "widgetTutorial.badge": "Widget tutorial",
  "widgetTutorial.skip": "Skip",
  "widgetTutorial.prev": "Back",
  "widgetTutorial.next": "Next",
  "widgetTutorial.done": "Get started",
  "widgetTutorial.welcome.title": "A tiny workspace on your screen",
  "widgetTutorial.welcome.body": "The Bubli widget floats today's tasks, timers, and memos on your screen without opening a browser. Here are the essentials in 30 seconds.",
  "widgetTutorial.bubble.title": "The bubble — always-there starting point",
  "widgetTutorial.bubble.body": "Keep the round bubble anywhere; click it to open the menu, and drag it to the spot you like.",
  "widgetTutorial.bar.title": "The bar — where folded widgets gather",
  "widgetTutorial.bar.body": "Fold a widget and it collapses into a slim bar as a chip. Hover a chip to peek at a preview.",
  "widgetTutorial.room.title": "Personal and project rooms stay separate",
  "widgetTutorial.room.body": "Switch personal/project room at the top and the widget follows that context. Timers and memos are personal-only.",
  "widgetTutorial.modes.title": "Default, translucent, ghost transparency",
  "widgetTutorial.modes.body": "Dial the widget's opacity across three levels so it never gets in the way. Ghost keeps only the numbers, like glass.",
  "widgetTutorial.ready.title": "You are ready!",
  "widgetTutorial.ready.body": "You can reopen the widget tutorial anytime from Settings > Display.",
  "widgetTutorial.scene.default": "Default",
  "widgetTutorial.scene.translucent": "Translucent",
  "widgetTutorial.scene.ghost": "Ghost",
  "widgetTutorial.scene.personal": "Personal",
  "widgetTutorial.scene.room": "Project room",

  "onboarding.settings.widgetReplayTitle": "Replay widget tutorial",
  "onboarding.settings.widgetReplayDesc": "Full-screen walkthrough of the desktop bubbles (bubble, bar, personal/room, transparency modes).",
  "onboarding.settings.widgetReplayCta": "Replay",
};

const ja: Record<Key, string> = {
  "onboarding.aria": "初回セットアップ",
  "onboarding.stepLabel": "{current} / {total} ステップ",
  "onboarding.back": "戻る",
  "onboarding.next": "次へ",
  "onboarding.skip": "スキップ",
  "onboarding.welcome.kicker": "Bubliをはじめる",
  "onboarding.welcome.title": "はじめまして、{name}さん!",
  "onboarding.welcome.body": "Bubliはプロジェクトルーム・予定・AI候補ボックスをひとつのワークスペースにまとめます。1分であなたに合ったホームを準備できます。",
  "onboarding.welcome.nameLabel": "表示名",
  "onboarding.welcome.nameHint": "ホームやプロジェクトルームでこの名前で表示されます。設定でいつでも変更できます。",
  "onboarding.role.title": "どんなお仕事をしていますか?",
  "onboarding.role.body": "職種に合わせてホームカードの構成と既定の開始画面をおすすめします。",
  "onboarding.role.gridAria": "職種を選択",
  "onboarding.role.developer": "開発者",
  "onboarding.role.designer": "デザイナー",
  "onboarding.role.pm": "PM・企画",
  "onboarding.role.marketer": "マーケター",
  "onboarding.role.writer": "ライター・コンテンツ",
  "onboarding.role.etc": "その他",
  "onboarding.preset.title": "{role}向けのセットが準備できました",
  "onboarding.preset.body": "以下の構成でホームを始めます。ホームのカード編集でいつでも変更できます。",
  "onboarding.preset.cardsLabel": "ホームカード構成",
  "onboarding.preset.homeLabel": "既定の開始画面",
  "onboarding.preset.homePersonal": "個人ホーム",
  "onboarding.preset.homeProjectRoom": "プロジェクトルーム",
  "onboarding.preset.apply": "この構成で始める",
  "onboarding.preset.applying": "適用中…",

  "onboarding.desktopOverlay.aria": "ウィジェット起動ガイド",
  "onboarding.desktopOverlay.progress": "{current} / {total} ステップ",
  "onboarding.desktopOverlay.next": "見る",
  "onboarding.desktopOverlay.skip": "スキップ",
  "onboarding.desktopOverlay.done": "はじめる",
  "onboarding.desktopOverlay.welcome.title": "Bubliへようこそ",
  "onboarding.desktopOverlay.welcome.body": "作業を邪魔しないデスクトップヘルパーです。30秒でポイントを確認できます。",
  "onboarding.desktopOverlay.bubble.title": "オーブ, 常に表示される起点",
  "onboarding.desktopOverlay.bubble.body": "画面上をゆっくり漂う丸いオーブです。タップするとメニューが開きます。",
  "onboarding.desktopOverlay.bar.title": "バー, 最小化したバブルをまとめる場所",
  "onboarding.desktopOverlay.bar.body": "最小化したバブルはここに収納されます。タップすると展開し、ホバーするとプレビューだけが表示されます（通知だけは即座に開きます）。",
  "onboarding.desktopOverlay.room.title": "個人とプロジェクトルームを分けています",
  "onboarding.desktopOverlay.room.body": "タイマー・メモは個人用です。コミュニケーションのようなウィジェットはプロジェクトルームに属します。ルーム選択は必須ではありません。",
  "onboarding.desktopOverlay.modes.title": "標準・半透明・ゴースト",
  "onboarding.desktopOverlay.modes.body": "ウィジェットの透明度を3段階で変えられます。ゴーストは数字などの要点だけをガラスのように残します。",
  "onboarding.desktopOverlay.environment.title": "作業環境を選んでください",
  "onboarding.desktopOverlay.environment.body": "ウィジェットの初期配置を選ぶと、そのまま整えます。バブルメニューの自動整列からいつでも変更できます。",
  "onboarding.desktopOverlay.environment.aria": "起動レイアウトの選択",
  "onboarding.desktopOverlay.environment.bar.title": "ミニマルバー",
  "onboarding.desktopOverlay.environment.bar.body": "細いバーだけを置いて、必要なときに展開します。",
  "onboarding.desktopOverlay.environment.board.title": "ウィジェットボード",
  "onboarding.desktopOverlay.environment.board.body": "タスク・予定・タイマーを画面右側に整えて置きます。",
  "onboarding.desktopOverlay.environment.cascade.title": "カードデッキ",
  "onboarding.desktopOverlay.environment.cascade.body": "主要ウィジェット3つを重ねてまとめます。",
  "onboarding.desktopOverlay.ready.title": "準備完了！",
  "onboarding.desktopOverlay.ready.body": "必要なら設定 > 表示から再表示できます。",

  "tour.aria": "ワークスペースツアー",
  "tour.progress": "{current} / {total}",
  "tour.next": "次へ",
  "tour.prev": "戻る",
  "tour.skip": "スキップ",
  "tour.done": "はじめる",
  "tour.welcome.title": "Bubliへようこそ",
  "tour.welcome.body": "Bubliは散らばった資料・タスク・予定をひとつにまとめるフリーランスの作業ハブです。主要メニューを順にご案内します。",
  "tour.sidebar.title": "サイドバー",
  "tour.sidebar.body": "左のサイドバーからすべての画面に移動できます。主要なタブを一つずつ見ていきましょう。",
  "tour.calendar.title": "カレンダー",
  "tour.calendar.body": "締め切りや予定を一目で確認し、ここから新しい予定を作成できます。",
  "tour.projectRooms.title": "プロジェクトルーム",
  "tour.projectRooms.body": "チームで使う作業スペースです。ルームごとにタスク・資料・チャットが集まります。",
  "tour.resources.title": "資料ボード",
  "tour.resources.body": "契約書や画像などのファイルをアップロードしてチームと共有します。",
  "tour.chat.title": "チャット",
  "tour.chat.body": "プロジェクトルームのメンバーとメッセージや音声で会話します。",
  "tour.roomSwitcher.title": "プロジェクトルーム切り替え",
  "tour.roomSwitcher.body": "上部で作業中のプロジェクトルームを切り替えたり、新しく作成したりできます。",
  "tour.cardEdit.title": "ホームカード編集",
  "tour.cardEdit.body": "ホームカードはドラッグで並べ替え、必要なカードだけ残せます。",
  "tour.agent.title": "AI候補ボックス",
  "tour.agent.body": "AIが提案したToDo・予定の候補がここに集まります。確認して承認すると反映されます。",

  "onboarding.settings.replayTitle": "会員サイトのチュートリアルをもう一度見る",
  "onboarding.settings.replayDesc": "ワークスペース案内(サイドバー・プロジェクトルーム・ホームカード・AI候補ボックス)を最初からもう一度見ます。",
  "onboarding.settings.replayCta": "もう一度見る",

  "widgetTutorial.aria": "ウィジェットチュートリアル",
  "widgetTutorial.badge": "ウィジェットチュートリアル",
  "widgetTutorial.skip": "スキップ",
  "widgetTutorial.prev": "戻る",
  "widgetTutorial.next": "次へ",
  "widgetTutorial.done": "はじめる",
  "widgetTutorial.welcome.title": "画面の上の小さな作業場、バブル",
  "widgetTutorial.welcome.body": "Bubliウィジェットは、ブラウザを開かなくても今日のタスク・タイマー・メモを画面上に浮かべます。30秒で要点だけご案内します。",
  "widgetTutorial.bubble.title": "バブル — いつでも浮かぶ出発点",
  "widgetTutorial.bubble.body": "丸いバブルは画面のどこにでも置けます。クリックでメニューが開き、ドラッグで好きな位置へ移せます。",
  "widgetTutorial.bar.title": "バー — 畳んだウィジェットが集まる場所",
  "widgetTutorial.bar.body": "使っていたウィジェットを畳むと、細いバーにチップとして集まります。チップに乗せるとプレビューだけ少し出ます。",
  "widgetTutorial.room.title": "個人とプロジェクトルームは分かれます",
  "widgetTutorial.room.body": "上部で個人/プロジェクトルームを切り替えると、ウィジェットの中身もその文脈に変わります。タイマーやメモは個人専用です。",
  "widgetTutorial.modes.title": "標準・半透明・ゴーストで透過を調整",
  "widgetTutorial.modes.body": "作業の邪魔にならないよう、ウィジェットの透明度を3段階で変えられます。ゴーストは数字だけをガラスのように残します。",
  "widgetTutorial.ready.title": "準備完了！",
  "widgetTutorial.ready.body": "ウィジェットチュートリアルは、設定 > 表示からいつでも再表示できます。",
  "widgetTutorial.scene.default": "標準",
  "widgetTutorial.scene.translucent": "半透明",
  "widgetTutorial.scene.ghost": "ゴースト",
  "widgetTutorial.scene.personal": "個人",
  "widgetTutorial.scene.room": "プロジェクトルーム",

  "onboarding.settings.widgetReplayTitle": "ウィジェットチュートリアルをもう一度見る",
  "onboarding.settings.widgetReplayDesc": "デスクトップバブル(バブル・バー・個人/ルーム・透過モード)の使い方を全画面でもう一度見ます。",
  "onboarding.settings.widgetReplayCta": "もう一度見る",
};

export const onboardingMessages = { ko, en, ja };
