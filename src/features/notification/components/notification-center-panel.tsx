...
const notifications: NotificationItem[] = [
  ...
  {
    description: "프로젝트룸 채팅에서 나를 언급했습니다. Tauri에서는 소통 버블에서 바로 열 수 있습니다.",
    kind: "communication",
  ...
  {
    description: "오후 일정이 30분 앞으로 다가왔습니다. Google Calendar와 연결된 일정은 서버 일정 기준으로 표시합니다.",
    kind: "schedule",
    origin: "schedules",
      ...
      <GlassPanel className="notification-center__hero">
        <div className="notification-center__title">
          ...
          <div>
            ...
            <h2>서버 알림을 모아 보고, 버블에서는 필요한 것만 띄웁니다</h2>
            <p>
              알림 원본은 서버 notifications에 두고, 읽음·숨김·고정 같은 사용자 상태는 항목 상태로 저장합니다.
            </p>
          </div>
        </div>
      ...
      <div className="notification-center__grid">
        <GlassPanel className="notification-center__list">
          ...
          <div className="notification-center__items">
            {notifications.map((item) => (
              <NotificationRow item={item} key={`${item.origin}-${item.title}`} />
            ))}
          </div>
        </GlassPanel>
        ...
        <GlassPanel className="notification-center__policy">
          ...
          <dl>
            <div>
              <dt>원본</dt>
              <dd>notifications와 각 도메인 원본 테이블을 기준으로 다시 불러옵니다.</dd>
            </div>
            <div>
              <dt>사용자 상태</dt>
              <dd>읽음, 숨김, 고정, 다시 보기는 widget_item_states에 갱신합니다.</dd>
            </div>
            <div>
              <dt>Tauri 표시</dt>
              <dd>알림 버블은 최근 표시 데이터만 캐시하고, 상세 사용 이벤트는 로컬에만 남깁니다.</dd>
            </div>
          </dl>
        </GlassPanel>
      </div>
