...
const routes: NotificationRoute[] = [
  {
    description: "알림 원본은 서버에 두고 사용자별 읽음과 보관 상태를 함께 관리합니다.",
    icon: Server,
    label: "서버 원본",
    owner: "notifications",
  ...
  },
  {
    ...
    icon: BellRing,
    label: "알림 버블",
    owner: "features/widget",
    route: "GET /api/widget/summary",
    tone: "personal",
  },
  {
    description: "Tauri 시스템 알림은 사용자가 켠 종류만 운영체제 알림으로 보냅니다.",
...
const notificationItems: NotificationItem[] = [
  {
    ...
    source: "AGENT_SUGGESTIONS_CREATED",
    state: "unread",
  },
  {
    channel: "버블",
  ...
  },
  {
    ...
    source: "RESOURCE_ANALYSIS_COMPLETED",
    state: "archived",
  },
];
...
const deliveryRules: DeliveryRule[] = [
  {
    ...
    label: "상태 변경",
    token: "PATCH /api/notifications/{id}/read",
  },
  {
    detail: "버블에는 서버 알림 전체가 아니라 지금 확인할 항목만 요약해 보여줍니다.",
    label: "버블 표시",
    token: "notification bubble",
  },
  {
    detail: "Tauri 시스템 알림은 앱 설정과 운영체제 권한을 모두 확인한 뒤 보냅니다.",
    label: "권한 확인",
    token: "Tauri notification",
  },
  {
    detail: "알림 상세 사용 이벤트는 로컬 집계 대상이며, 서버에는 항목 상태와 요약 집계만 보냅니다.",
    label: "사용 기록",
    token: "widget_daily_summaries",
  },
];
    ...
    <article className={styles.routeCard}>
      ...
      <div>
        <div className={styles.badges}>
          ...
          <StatusBadge tone="neutral">{item.route}</StatusBadge>
        </div>
        <h3>{item.label}</h3>
    ...
    <section className={styles.panel} aria-label="알림 라우팅 경계 패널">
      <GlassPanel className={styles.hero}>
        <div className={styles.heroCopy}>
          ...
          <h2>알림 원본은 서버에 두고, 화면과 버블은 필요한 만큼만 보여줍니다</h2>
          <p>
            Bubli 알림은 웹 알림 센터, Tauri 시스템 알림, 버블 표시가 같은 원본을 봅니다. 사용자가 읽거나
      ...
      <div className={styles.columns}>
        <GlassPanel className={styles.listPanel}>
          ...
          <div className={styles.notificationList}>
            {notificationItems.map((item) => (
              <NotificationRow item={item} key={`${item.channel}-${item.source}`} />
            ))}
          </div>
        </GlassPanel>
        ...
        <GlassPanel className={styles.rulePanel}>
          ...
          <Chip icon={<CheckCircle2 size={14} />}>웹, Tauri, 버블이 같은 알림 원본을 사용</Chip>
        </GlassPanel>
      </div>
      ...
      <GlassPanel className={styles.footerPanel}>
        ...
        <StatusBadge tone="approved">notifications</StatusBadge>
        <StatusBadge tone="pending">/user/queue/notifications</StatusBadge>
      </GlassPanel>
    </section>
  );
}
