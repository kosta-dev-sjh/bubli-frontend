...
// 2) 회원사이트 튜토리얼(코치 마크): 온보딩 직후 이어서 보여주고, 설정 > 표시의
//    "회원사이트 튜토리얼 다시 보기"(OPEN_TUTORIAL_EVENT)로 언제든 다시 열 수 있다.
// 3) 위젯 튜토리얼: 데스크탑(macOS)에서는 코치 마크가 끝난 뒤 전체화면 오버레이 창으로 이어지고
//    (마지막 단계에서 시작 환경까지 구성), 웹에서는 전체화면 모달(WidgetTutorial)로 미리 볼 수 있다.
//    설정 > 표시의 "위젯 튜토리얼 다시 보기"(OPEN_WIDGET_TUTORIAL_EVENT)로 다시 연다.
...
type FirstRunPhase = "idle" | "onboarding" | "tour" | "widgetTutorial";
...
export function FirstRunController({ user }: FirstRunControllerProps) {
  ...
  // 첫 실행 흐름에서만 코치 마크 종료 후 위젯 튜토리얼(오버레이)로 이어간다.
  const [chainWidgetTutorial, setChainWidgetTutorial] = useState(false);

  const openWidgetTutorial = useCallback(() => {
    // 데스크탑(macOS)은 전용 전체화면 오버레이 창(환경 구성 포함), 그 외에는 전체화면 모달.
    if (isMacTauriRuntime()) {
      void tauriCommands.openOnboardingOverlay();
      return;
    }
    setPhase("widgetTutorial");
  }, []);
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      ...
      if (stored?.userId === user.id && !stored.tutorialCompletedAt) {
        ...
        // (다시 보기는 설정 > 표시의 수동 경로만 사용한다.)
        completeTutorial(user.id);
        setChainWidgetTutorial(true);
        setPhase("tour");
      }
    }, 0);
  ...
  // 설정 > 표시 "회원사이트 튜토리얼 다시 보기" — AppShell에 상주하므로 어느 화면에서든 받는다.
  useEffect(() => {
    function openTutorial() {
      setChainWidgetTutorial(false);
  ...
  // 설정 > 표시 "위젯 튜토리얼 다시 보기".
  useEffect(() => {
    function onOpenWidgetTutorial() {
      openWidgetTutorial();
    }

    window.addEventListener(OPEN_WIDGET_TUTORIAL_EVENT, onOpenWidgetTutorial);
    return () => window.removeEventListener(OPEN_WIDGET_TUTORIAL_EVENT, onOpenWidgetTutorial);
  }, [openWidgetTutorial]);
  ...
  const handleTourClose = () => {
    ...
    completeTutorial(user.id);
    setPhase("idle");
    // 첫 실행 흐름이면 데스크탑에서 위젯 튜토리얼(환경 구성 포함)로 이어간다. 웹 첫 실행은
    // 상주 위젯이 없으므로 이어가지 않고, 설정의 다시 보기로만 미리 볼 수 있게 둔다.
    if (chainWidgetTutorial) {
      setChainWidgetTutorial(false);
      if (isMacTauriRuntime()) {
        void tauriCommands.openOnboardingOverlay();
      }
  ...
  if (phase === "widgetTutorial") {
    return <WidgetTutorial onClose={() => setPhase("idle")} />;
  }
