...
function ContractCard({ item }: { item: ContractArea }) {
  ...
  return (
    <article className={styles.contractCard}>
      ...
      <ProgressBar label={`${item.label} 업무 기준 반영률`} value={item.progress} />
    </article>
  );
}
    ...
    <section className={styles.panel} aria-label="API 기준 상태 패널">
      <GlassPanel className={styles.hero}>
        <div>
          ...
          <h2>프론트는 확정된 API 기준을 기준으로 연결하고, 대기 항목은 표시해 둡니다</h2>
          <p>
            인증, 채팅, 보이스챗, 에이전트 작업, Tauri 동기화는 API 기준의 저장 기준을 따릅니다. 백엔드
            산출물과 응답 샘플이 확인되면 프론트 연결부에 반영합니다.
          </p>
        </div>
        <div className={styles.summary}>
          <StatusBadge tone="approved">업무 기준 기준</StatusBadge>
          <strong>7개</strong>
          <span>연동 축 확인</span>
