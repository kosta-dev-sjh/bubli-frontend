...
function ContractCard({ item }: { item: ContractArea }) {
  ...
  return (
    <article className={styles.contractCard}>
      ...
      <ProgressBar label={`${item.label} API 기준 반영률`} value={item.progress} />
    </article>
  );
}
    ...
    <section className={styles.panel} aria-label="API 기준 상태 패널">
      <GlassPanel className={styles.hero}>
        <div>
          ...
          <p>
            인증, 채팅, 보이스챗, 에이전트 작업, Tauri 동기화는 API 응답과 저장 기준을 따릅니다. 백엔드
            산출물과 응답 샘플이 확인되면 프론트 연결부에 반영합니다.
          </p>
        </div>
        <div className={styles.summary}>
          <StatusBadge tone="approved">확정 기준</StatusBadge>
          <strong>7개</strong>
          <span>연동 축 확인</span>
