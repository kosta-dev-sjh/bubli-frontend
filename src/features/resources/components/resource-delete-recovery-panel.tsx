...
const statusMeta: Record<DeleteRecoveryStatus, { label: MessageKey; tone: StatusTone }> = {
  deleteCandidate: { label: "resources.delete.statusDeleteCandidate", tone: "warning" },
  archived: { label: "resources.delete.statusArchived", tone: "neutral" },
  ready: { label: "resources.delete.statusReady", tone: "success" },
  blocked: { label: "resources.delete.statusBlocked", tone: "pending" },
};

const actionMeta: Record<DeleteRecoveryAction, { icon: ReactNode; label: MessageKey }> = {
  keep: {
    icon: <Undo2 size={15} strokeWidth={2.1} />,
    label: "resources.delete.actionKeep",
  },
  archive: {
    icon: <Archive size={15} strokeWidth={2.1} />,
    label: "resources.delete.actionArchive",
  },
  confirmDelete: {
    icon: <Trash2 size={15} strokeWidth={2.1} />,
    label: "resources.delete.actionConfirmDelete",
  },
};
    ...
    <GlassPanel as="section" className={cn(styles.panel, className)} {...props}>
      <header className={styles.header}>
        <div className={styles.titleBlock}>
          ...
          <div>
            <h2 className={styles.title}>{title ?? t("resources.delete.defaultTitle")}</h2>
