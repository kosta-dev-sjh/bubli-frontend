...
function formatTime(event: ScheduleResponse, t: Translate) {
...
function CalendarPageContent() {
  ...
  const [draftTitle, setDraftTitle] = useState(() => t("calendar.draft.defaultTitle"));
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      ...
      setDraftNotice(t("calendar.draft.created"));
          ...
          {composerOpen ? (
            <div className={styles.composerLayer} role="presentation" onMouseDown={() => setComposerOpen(false)}>
              ...
              >
                <div className={styles.panelHeader}>
                  <div>
                    <h2 id="calendar-composer-title">{t("calendar.composer.title")}</h2>
                    <p>{t("calendar.composer.subtitle")}</p>
                  </div>
                  <button aria-label={t("calendar.composer.close")} className={styles.composerClose} onClick={() => setComposerOpen(false)} type="button">
                ...
                <section className={styles.repeatBox} aria-label={t("calendar.composer.repeatAria")}>
                  ...
                  <div className={repeatEnabled ? styles.repeatControls : `${styles.repeatControls} ${styles.repeatControlsDisabled}`}>
                    <label className={styles.field}>
                      ...
                      >
                        {(Object.entries(repeatLabelKeys) as Array<[RepeatInterval, MessageKey]>).map(([value, labelKey]) => (
