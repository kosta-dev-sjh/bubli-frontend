...
export function ResourceFlowView({
  ...
  const hasData = Boolean(data?.resources.length);
  ...
  const pendingCount = data?.suggestions.filter((suggestion) => suggestion.status === "pending").length ?? 0;
  const roomCount = data?.resources.filter((resource) => resource.scope === "room").length ?? 0;
  ...
  if (empty || !hasData || !data || !selected) {
    return (
      <GlassPanel className={styles.statePanel}>
